...
void input_setup() {
  pinMode(VRX, INPUT);
  pinMode(VRY, INPUT);
  pinMode(SW, INPUT_MODE);
  digitalWrite(SW, HIGH);
}

bool input_left() {
  return analogRead(VRY) < 1000;
};

bool input_right() {
  return analogRead(VRY) > 3000;
};

bool input_up() {
  return analogRead(VRX) > 3000;
};

bool input_down() {
  return analogRead(VRX) < 1000;
};

bool input_fire() {
  return digitalRead(SW) == INPUT_STATE;
};
#endif
